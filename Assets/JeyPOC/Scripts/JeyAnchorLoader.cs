...
/// </remarks>
public class JeyAnchorLoader : MonoBehaviour
{
    [SerializeField]
    OVRSpatialAnchor _anchorPrefab; // for loaded instantiate

    public OVRSpatialAnchor[] _Anchor;
    ...
    private void OnLocalized(OVRSpatialAnchor.UnboundAnchor unboundAnchor, bool success)
    {
        ...
        Debug.Log("UUID is " + PlayerPrefs.GetString(unboundAnchor.Uuid.ToString()));

        switch (PlayerPrefs.GetString(unboundAnchor.Uuid.ToString()))
        {
            case "0":
                _anchorPrefab = _Anchor[0];
                break;
            case "1":
                _anchorPrefab = _Anchor[1];
                break;
            case "2":
                _anchorPrefab = _Anchor[2];
                break;
            default:
                _anchorPrefab = _Anchor[1];
                Debug.Log(PlayerPrefs.GetString(unboundAnchor.Uuid.ToString()));
                break;
        }
